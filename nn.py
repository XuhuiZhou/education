...
class TriAAttention(nn.Module):
    '''
    Bi-Affine attention layer.
    '''

    def __init__(self, out_dim ,input_size_encoder, input_size_decoder, num_labels, biaffine=True, **kwargs):
        '''
        ...
        Args:
            ...
            **kwargs:
        '''
        super(TriAAttention, self).__init__()
        self.input_size_encoder = input_size_encoder
        self.input_size_decoder = input_size_decoder
        self.num_labels = num_labels
        self.biaffine = biaffine
        self.head_p = nn.Linear(input_size_encoder, 128)
        self.dep_p = nn.Linear(input_size_encoder, 128)
        self.g_p = nn.Linear(out_dim, 128)
        ...
        if self.biaffine:
            self.U = Parameter(torch.Tensor(self.num_labels, 128,128,128))
        else:
    ...
    def forward(self, input_d, input_e, g, mask_d=None, mask_e=None):
        '''
        ...
        if self.biaffine:
            input_d = F.elu(self.head_p(input_d))
            input_e = F.elu(self.dep_p(input_e))
            g = F.elu(self.g_p(g)).unsqueeze(1)
            ...
            #print(g.shape)
            #rint(g.unsqueeze(3).shape)
            output = output.transpose(1,3)
            #print(output.shape)
            output = torch.matmul(output, g.unsqueeze(3))
            output = output.transpose(1,3)
