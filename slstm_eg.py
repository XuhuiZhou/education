...
class SLSTM_EG(nn.Module):
    ...
    def forward(self, word_inputs, mask, num_layers):
        ...
        dummynode_hidden_states = initial_hidden_states # batch_size * hidden_dim
        
        # self.debug = dummynode_hidden_states
        ##print("[tlog] dummynode_hidden_states: " + str(dummynode_hidden_states))

        dummynode_cell_states = initial_cell_states
        ...
        for i in range(num_layers):
            ...
            reshaped_hidden_output = torch.unsqueeze(initial_hidden_states, dim=1).repeat(1,shape[1],1,1)
            # copy dummy states for computing forget gate

            transformed_dummynode_hidden_states = torch.unsqueeze(dummynode_hidden_states, dim=2).repeat(1,1,shape[1], 1)
            ...
            ########################################Computation for global node


            gated_d_t = torch.sigmoid(
                torch.matmul(dummynode_hidden_states, self.gated_Wxd) + torch.matmul(initial_hidden_states,
            ...
            gated_o_t = torch.sigmoid(
                torch.matmul(dummynode_hidden_states, self.gated_Wxo) + torch.matmul(initial_hidden_states,
            ...
            reshaped_gated_f_t = gated_f_t + torch.unsqueeze(mask_softmax_score_expanded, dim =3)
            ##print("[tlog] reshaped_gated_f_t: " + str(reshaped_gated_f_t))

            gated_softmax_scores = F.softmax(
                torch.cat([reshaped_gated_f_t, torch.unsqueeze(gated_d_t, dim=2)], dim=2), dim=2)
            ##print("[tlog] gated_softmax_scores: " + str(gated_softmax_scores))

            # self.debug = gated_softmax_scores
            # split the softmax scores
            new_reshaped_gated_f_t = gated_softmax_scores[:, :,:shape[1]*shape[1], :]
            new_gated_d_t = gated_softmax_scores[:, :,shape[1]*shape[1]:, :]
            ##print("[tlog] new_reshaped_gated_f_t: " + str(new_reshaped_gated_f_t))
            ##print("[tlog] new_gated_d_t: " + str(new_gated_d_t))
            reshaped_initial_cell_states = torch.unsqueeze(initial_cell_states, dim=1).repeat(1,shape[1],1,1)
            # new dummy states
            dummy_c_t = torch.sum(new_reshaped_gated_f_t * reshaped_initial_cell_states, dim=2) + torch.squeeze(new_gated_d_t, dim=2) * dummynode_cell_states
            ...
            transformed_dummynode_hidden_states = dummynode_hidden_states.view(-1, hidden_size)
            transformed_dummynode_cell_states = dummynode_cell_states.view(-1, hidden_size)
            ...
            dummynode_hidden_states = dummy_h_t * sequence_mask
            dummynode_cell_states = dummy_c_t * sequence_mask
